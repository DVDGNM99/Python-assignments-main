...
def move_snake(head, dx, dy):
    """Calculate the new head position."""
    return (head[0] + dx, head[1] + dy)

def check_collision(head, snake_body):
    """Check if the head collides with walls or self."""
    if (head[0] < 0 or head[0] >= WIDTH or         # Check collision with left or right borders
        ...
        head in snake_body):                       # Check collision with its own body
        return True
    return False

def check_eating(head, food):
